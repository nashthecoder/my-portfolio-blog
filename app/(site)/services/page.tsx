  ...
  return (
    <div style={{ background: C.warmBg, minHeight: '100vh' }}>
      <div className="page-container py-12">
        ...
        <p style={{ fontSize:'11px', fontWeight:'500', color:C.crimson, textTransform:'uppercase', letterSpacing:'0.09em', marginBottom:'0.4rem' }}>Services</p>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,minmax(0,1fr))', gap:'12px', marginBottom:'3rem' }}>
          {SERVICES.map(({ title, desc }) => (
            <div key={title} style={{ padding:'1.25rem 1.25rem 1.25rem 1.1rem', background:C.surface, borderRadius:'0 12px 12px 0', border:`1px solid ${C.border}`, borderLeft:`3px solid ${C.crimson}` }}>
              <p style={{ fontSize:'14px', fontWeight:'500', color:C.charcoal, marginBottom:'6px' }}>{title}</p>
        ...
        <div style={{ background:C.crimsonLight, border:'1px solid #F5C4BE', borderRadius:'16px', padding:'2rem', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'1.5rem', flexWrap:'wrap' }}>
          <div>
            <p style={{ fontSize:'15px', fontWeight:'500', color:C.crimsonDark, marginBottom:'4px' }}>Ready to work together?</p>
            <p style={{ fontSize:'14px', color:C.muted, margin:0, lineHeight:1.6 }}>Tell me what you&apos;re building. I&apos;ll tell you how I can help.</p>
          </div>
          <a href="https://quickjobs.naijeriatoweett.com/" target="_blank" rel="noopener noreferrer"
            style={{ fontSize:'13px', fontWeight:'500', padding:'0.65rem 1.5rem', borderRadius:'100px', background:C.crimson, color:'#fff', textDecoration:'none', flexShrink:0 }}>
