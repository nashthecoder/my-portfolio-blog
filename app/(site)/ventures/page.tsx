  ...
  return (
    <div style={{ background: C.warmBg, minHeight: '100vh' }}>
      <div className="page-container py-12">
        ...
        <p style={{ fontSize:'11px', fontWeight:'500', color:C.crimson, textTransform:'uppercase', letterSpacing:'0.09em', marginBottom:'0.4rem' }}>My ventures</p>
        ...
        <p style={{ fontSize:'15px', color:C.muted, marginBottom:'3rem', lineHeight:1.7, maxWidth:'52ch' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'1.5rem' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'240px 1fr', borderRadius:'18px', overflow:'hidden', border:`1px solid ${C.charcoal}` }}>
            <div style={{ background:C.charcoalMid, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'10px', padding:'2rem', minHeight:'240px' }}>
              ...
              <span style={{ fontSize:'10px', color:'rgba(255,255,255,0.25)', textAlign:'center', lineHeight:1.5, textTransform:'uppercase', letterSpacing:'0.06em' }}>TichLabs studio or team photo{'\n'}800×1067px{'\n'}/public/images/venture-tich.jpg</span>
            </div>
            <div style={{ background:C.charcoal, padding:'2rem', display:'flex', flexDirection:'column', gap:'0.9rem' }}>
              <p style={{ fontSize:'10px', fontWeight:'500', color:C.crimsonMid, textTransform:'uppercase', letterSpacing:'0.07em' }}>Active · Feminist tech studio</p>
              <h2 style={{ fontFamily:'"DM Serif Display",Georgia,serif', fontSize:'1.75rem', color:'#fff', lineHeight:1.15, margin:0 }}>TichLabs&nbsp;<em style={{ color:C.crimsonMid }}>/&nbsp;TichLabs</em></h2>
              <p style={{ fontSize:'13.5px', color:'rgba(255,255,255,0.55)', lineHeight:1.7 }}>{VENTURES[0].description}</p>
              <div style={{ display:'flex', flexWrap:'wrap', gap:'5px' }}>
                {VENTURES[0].tags.map(t => (
                  <span key={t} style={{ fontSize:'10px', padding:'2px 9px', borderRadius:'100px', background:'rgba(255,255,255,0.07)', color:'rgba(255,255,255,0.55)' }}>{t}</span>
                ))}
              </div>
              <a href="/work" style={{ fontSize:'13px', fontWeight:'500', color:C.crimsonMid, textDecoration:'none' }}>See our work →</a>
            </div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 240px', borderRadius:'18px', overflow:'hidden', border:`1px solid ${C.burgundy}` }}>
            <div style={{ background:'#fff', padding:'2rem', display:'flex', flexDirection:'column', gap:'0.9rem' }}>
              <p style={{ fontSize:'10px', fontWeight:'500', color:C.burgundy, textTransform:'uppercase', letterSpacing:'0.07em' }}>In development · FemTech</p>
              <h2 style={{ fontFamily:'"DM Serif Display",Georgia,serif', fontSize:'1.75rem', color:C.charcoal, lineHeight:1.15, margin:0 }}>DADA — <em style={{ color:C.burgundy }}>menopause companion</em></h2>
              <p style={{ fontSize:'13.5px', color:C.muted, lineHeight:1.7 }}>{VENTURES[1].description}</p>
              <div style={{ display:'flex', flexWrap:'wrap', gap:'5px' }}>
                {VENTURES[1].tags.map(t => (
                  <span key={t} style={{ fontSize:'10px', padding:'2px 9px', borderRadius:'100px', background:C.burgundyLight, color:C.burgundy }}>{t}</span>
                ))}
              </div>
              <a href="#" style={{ fontSize:'13px', fontWeight:'500', color:C.burgundy, textDecoration:'none' }}>Follow the build →</a>
            </div>
            <div style={{ background:C.burgundyLight, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'10px', padding:'2rem' }}>
              ...
              <span style={{ fontSize:'10px', color:C.burgundy, textAlign:'center', lineHeight:1.5, textTransform:'uppercase', letterSpacing:'0.06em', opacity:0.6 }}>DADA brand image{'\n'}800×1067px{'\n'}/public/images/venture-dada.jpg</span>
            </div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'240px 1fr', borderRadius:'18px', overflow:'hidden', border:`1px solid ${C.crimson}` }}>
            <div style={{ background:C.crimsonLight, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'10px', padding:'2rem' }}>
              ...
              <span style={{ fontSize:'10px', color:C.crimsonDark, textAlign:'center', lineHeight:1.5, textTransform:'uppercase', letterSpacing:'0.06em', opacity:0.7 }}>Community photo{'\n'}800×1067px{'\n'}/public/images/venture-teaches.jpg</span>
            </div>
            <div style={{ background:'#fff', padding:'2rem', display:'flex', flexDirection:'column', gap:'0.9rem' }}>
              <p style={{ fontSize:'10px', fontWeight:'500', color:C.crimson, textTransform:'uppercase', letterSpacing:'0.07em' }}>Active · EdTech</p>
              <h2 style={{ fontFamily:'"DM Serif Display",Georgia,serif', fontSize:'1.75rem', color:C.charcoal, lineHeight:1.15, margin:0 }}>Mama Tech Teaches — <em style={{ color:C.crimson }}>Build. Belong. Become.</em></h2>
              <p style={{ fontSize:'13.5px', color:C.muted, lineHeight:1.7 }}>{VENTURES[2].description}</p>
              <div style={{ display:'flex', flexWrap:'wrap', gap:'5px' }}>
                {VENTURES[2].tags.map(t => (
                  <span key={t} style={{ fontSize:'10px', padding:'2px 9px', borderRadius:'100px', background:C.crimsonLight, color:C.crimsonDark }}>{t}</span>
                ))}
              </div>
              <a href="https://mt-teaches.lovable.app/" target="_blank" rel="noopener noreferrer" style={{ fontSize:'13px', fontWeight:'500', color:C.crimson, textDecoration:'none' }}>Join the community →</a>
            </div>
          </div>
